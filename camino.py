...
def cutout_around_defocused_psf_multi(img,
                                size=128,
    ...
    for rec_size in recenter_sizes:
        ...
        yc, xc = estimate_center(cut, smooth_sigma=smooth_sigma, thresh_sigma=thresh_sigma)
        y, x = y1 + yc, x1 + xc
...
def cutout_around_defocused_psf(img, size=128, smooth_sigma=2.0, thresh_sigma=5.0):
    ...
    pad_y2 = max(0, y2 - img.shape[0]); pad_x2 = max(0, x2 - img.shape[1])
    ...
    cut = img[y1:y2, x1:x2]
    if any(p > 0 for p in [pad_y1, pad_y2, pad_x1, pad_x2]):
        cut = onp.pad(cut, ((pad_y1, pad_y2), (pad_x1, pad_x2)),
...
def _find_existing_cals(download_dir: str, want_prefix: str, det_tag: str | None):
    ...
    pattern = os.path.join(os.path.abspath(download_dir), "**", f"{want_prefix}*_cal.fits")
...
def err_poisson_dn(expected_dn=None,
                   gain_e_per_dn=1.0,
                   read_noise_e=0.0,
                   floor_e=1.0,
    ...
    if expected_dn is not None:
        ...
        var_e = onp.clip(expected_dn * gain, 0, None) + read_noise_e**2 + float(max(floor_e, 0.0))**2
...
class Rotate:
    ...
    def apply(self, PSF_or_array: Any) -> Any:
        ...
        sample_coords = rot_coords * jnp.array([1.0, self.anisotropy], dtype=jnp.float32)[:, None, None]
...
def map_coordinates_2d(
    ...
    if order not in (0, 1):
        raise NotImplementedError("This minimal version supports order 0 (nearest) or 1 (linear).")
...
class PixelAnisotropy(dl.layers.detector_layers.DetectorLayer):
    ...
    def apply(self, PSF):
        ...
        interp_fn = lambda x: map_coordinates_2d(x, coords, order=3, mode="constant", cval=0.0)
...
class ModelFit(zdx.Base):
    ...
    def map_param(self, exposure, param):
        ...
        """
        if param in ["fluxes", "positions","spectrum", "aberrations", "defocus"]: #, "aberrations", "cold_mask_shift", "cold_mask_rot", "cold_mask_scale", "cold_mask_shear", "primary_rot", "primary_scale", "primary_shear", "breathing", "slope", "spectrum"]:
...
def update_optics(self, model, exposure):
    ...
    if "aberrations" in model.params:
        ...
        pmask = (optics.layers["pupil"].transmission > 0).astype(jnp.float64) #opd_nm.dtype
...
class SinglePointFilterFit(ModelFit):
    ...
    def update_optics(self, model, exposure):
        ...
        # ----------------------------
        if "defocus" in model.params.keys():
            disp = model.defocus[self.get_key(exposure, "defocus")]  # nominal defocus param
    ...
    def __call__(self, model, exposure):
        ...
        if data.ndim == 3:
            ...
            raise ValueError(f"Unexpected PSF data ndim={data.ndim}, shape={data.shape}")
...
def get_filter(file_path, nwavels=1):
    ...
    wl_array, throughput_array = np.array(onp.loadtxt(file_path, unpack=True)) #the F444W filter is the NIRCam one
...
if p.exists():
    filter_files = {
    'F212N': get_filter_test(("/fred/oz440/shrish/deps/amigo/amigo/data/filters/F212N.dat"))[::5,:],
}
else:
    filter_files = {
    'F212N': get_filter_test(str(files("amigo").joinpath("data/filters/F212N.dat")))[::5,:],
}
...
def check_convergence_from_file(params, model_defocus, pup, fname, nw_list=(5, 10), crop_to=128):
    ...
    for nw in list(nw_list)[:-1]:
        diff = np.sqrt(np.mean((images[nw] - ref_img) ** 2)) / (np.mean(ref_img) + 1e-30)
...
def check_poly_vs_mono(model, exposure, nw=20, plot=False, tol=1e-3, label=""):
    ...
    try:
        ...
        print(f"[{label}] No 'spectrum' param for key {key} → likely MONO (only flux × filter).")
        return {"mode": "mono", "reason": "missing spectrum param", "key": key, "label": label}
    ...
    if plot:
        ...
        plt.title(f"{label} {exposure.filter}: {'POLY' if mode=='poly' else 'MONO-like'} (nw={nw})")
